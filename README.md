...
**Website & docs:** [jgsystemsconsulting.github.io/jgs-voiceflow](https://jgsystemsconsulting.github.io/jgs-voiceflow/)
· [Guide](https://jgsystemsconsulting.github.io/jgs-voiceflow/guide.html)
· [Docs](https://jgsystemsconsulting.github.io/jgs-voiceflow/docs.html)
...
`VoiceFlow-Setup.exe` (about 700 MB; the offline speech models are bundled, so the download is
large but you can dictate with no internet afterward, and it may take a few minutes).

Run the installer (per-user; no administrator rights needed). **Windows will warn about an unknown
publisher** the first time: choose **More info, then Run anyway**. VoiceFlow is a free release and
is not code-signed yet, so this warning is expected (see the [quickstart](docs/quickstart.md) for
the exact screen). You can [verify your download's checksum](docs/verifying-downloads.md) first.
